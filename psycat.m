...
[~,~,invdef,~,~,~]=LTNP_cat12_segment(T1, outfolder, voxelsize, WMHC, atlas); % also creates the deformation field (warping parameters) to MNI
...
% Make tissue masks
[GMmask_path, WMmask_path, CSFmask_path,~,~]=LTNP_make_labelimage(GM_path,WM_path,CSF_path,outfolder);

% Make final atlas based segmentation
atlas_path=fullfile(outfolder,'mri_atlas',[atlas '_' T1name T1ext]);
[~,~,~,segmentation]=LTNP_create_rbv_segm(GMmask_path, WMmask_path, CSFmask_path, atlas_path, outfolder);
