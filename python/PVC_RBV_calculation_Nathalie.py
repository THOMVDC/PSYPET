...
def load_data(fname):
    nii = nib.load(fname)
#    nii = nib.as_closest_canonical(nii) #rearrange the image voxel axes to make them as close as possible to RAS+ orientation
...
def rbv_pvc(pet_dir,seg_dir,output_pvc,fwhm):
    ...
    # Save RBV image
    rbv_dir = os.path.join(output_pvc,output_prefix + '_' + str(fwhm).replace('.','_') + 'mm_' + os.path.basename(pet_dir))
